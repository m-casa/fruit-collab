...
namespace HeathenEngineering.SteamworksIntegration.API
{
    ...
    public static class Matchmaking
    {
        ...
        public static class Client
        {
            ...
            /// </code>
            /// </param>
            public static void CreateLobby(ELobbyType type, int maxMembers, Action<EResult, Lobby, bool> callback)
            {
                if (callback == null)
                    return;
        ...
        public static class Client
        {
            ...
            public static LobbyEnterEvent EventLobbyEnter
            {
                get
                {
                    SteamMatchmaking.OnLobbyEntered -= FacepunchOnLobbyEnter;
                    SteamMatchmaking.OnLobbyEntered += FacepunchOnLobbyEnter;

                    return eventLobbyEnter;
                }
            }
            private static void FacepunchOnLobbyEnter(Steamworks.Data.Lobby lobby)
            {
                eventLobbyEnter?.Invoke(lobby);
            }
            /// <summary>
            /// Occurs when a lobby enter callback is recieved and the responce code is a success
            /// </summary>
            ...
            /// You can cast the m_EChatRoomEnterResponse value to a EChatRoomEnterResponse enum to determin the reason for failure
            /// </remarks>
            [System.Obsolete("You are useing Facepunch which does not support this feature, if you require this feature then remove Facepunch and install Steamworks.NET")]
            public static LobbyEnterEvent EventLobbyEnterFailed
            {
                get
                {
            ...
            public static LobbyDataUpdateEvent EventLobbyDataUpdate
            {
                get
                {
                    SteamMatchmaking.OnLobbyMemberDataChanged -= FacepunhcHandleLobbyMemberDataChange;
                    SteamMatchmaking.OnLobbyMemberDataChanged += FacepunhcHandleLobbyMemberDataChange;

                    SteamMatchmaking.OnLobbyDataChanged -= FacepunhcHandleLobbyDataChange;
                    SteamMatchmaking.OnLobbyDataChanged += FacepunhcHandleLobbyDataChange;

                    return eventLobbyDataUpdate;
                }
            }
            private static void FacepunhcHandleLobbyDataChange(Steamworks.Data.Lobby lobby)
            {
                eventLobbyDataUpdate?.Invoke(new LobbyDataUpdate
                {
                    lobby = lobby,
                    member = null
                });
            }
            private static void FacepunhcHandleLobbyMemberDataChange(Steamworks.Data.Lobby lobby, Friend friend)
            {
                eventLobbyDataUpdate?.Invoke(new LobbyDataUpdate
                {
                    lobby = lobby,
            ...
            public static LobbyChatMsgEvent EventLobbyChatMsg
            {
                get
                {
                    SteamMatchmaking.OnChatMessage -= FacepunchHandleLobbyChat;
                    SteamMatchmaking.OnChatMessage += FacepunchHandleLobbyChat;

                    return eventLobbyChatMsg;
                }
            }
            private static void FacepunchHandleLobbyChat(Steamworks.Data.Lobby lobby, Friend friend, string message)
            {
                eventLobbyChatMsg.Invoke(new LobbyChatMsg
                {
                    lobby = lobby,
            ...
            /// <param name="queryPort">The port used to query the server, in host order.</param>
            /// <param name="lastPlayedOnServer"></param>
            public static void AddHistoryGame(AppId appID, uint ipAddress, ushort port, ushort queryPort, DateTime lastPlayedOnServer) => SteamMatchmaking.AddFavoriteGame(appID, ipAddress, port, queryPort, Constants.k_unFavoriteFlagHistory, Convert.ToUInt32((lastPlayedOnServer - new DateTime(1970, 1, 1)).TotalSeconds));
            /// <summary>
            /// Adds the game server to the local favorites list or updates the time played of the server if it already exists in the list.
            /// </summary>
            /// <param name="appID">The App ID of the game.</param>
            ...
            /// <param name="queryPort">The port used to query the server, in host order.</param>
            /// <param name="lastPlayedOnServer"></param>
            public static void AddFavoriteGame(AppId appID, uint ipAddress, ushort port, ushort queryPort, DateTime lastPlayedOnServer) => SteamMatchmaking.AddFavoriteGame(appID, ipAddress, port, queryPort, Constants.k_unFavoriteFlagFavorite, Convert.ToUInt32((lastPlayedOnServer - new DateTime(1970, 1, 1)).TotalSeconds));
            /// <summary>
            /// Adds the game server to the local favorites list or updates the time played of the server if it already exists in the list.
            /// </summary>
            /// <param name="appID">The App ID of the game.</param>
            ...
            /// <param name="queryPort">The port used to query the server, in host order.</param>
            /// <param name="lastPlayedOnServer"></param>
            public static void AddHistoryGame(AppId appID, string ipAddress, ushort port, ushort queryPort, DateTime lastPlayedOnServer) => SteamMatchmaking.AddFavoriteGame(appID, Utilities.IPStringToUint(ipAddress), port, queryPort, Constants.k_unFavoriteFlagHistory, Convert.ToUInt32((lastPlayedOnServer - new DateTime(1970, 1, 1)).TotalSeconds));
            /// <summary>
            /// Adds the game server to the local favorites list or updates the time played of the server if it already exists in the list.
            /// </summary>
            /// <param name="appID">The App ID of the game.</param>
            ...
            /// <param name="queryPort">The port used to query the server, in host order.</param>
            /// <param name="lastPlayedOnServer"></param>
            public static void AddFavoriteGame(AppId appID, string ipAddress, ushort port, ushort queryPort, DateTime lastPlayedOnServer) => SteamMatchmaking.AddFavoriteGame(appID, Utilities.IPStringToUint(ipAddress), port, queryPort, Constants.k_unFavoriteFlagFavorite, Convert.ToUInt32((lastPlayedOnServer - new DateTime(1970, 1, 1)).TotalSeconds));
            /// <summary>
            /// Sets the physical distance for which we should search for lobbies, this is based on the users IP address and a IP location map on the Steam backed.
            /// </summary>
            /// <param name="distanceFilter">Specifies the maximum distance.</param>
            ...
            public static void CreateLobby(ELobbyType type, int maxMembers, Action<Lobby, bool> callback)
            {
                ...
                m_LobbyCreated_t.Set(handle, (r, e) =>
                {
                    if (!e && r.m_eResult == EResult.k_EResultOK)
                    {
                        SetLobbyData(new SteamId(r.m_ulSteamIDLobby), Lobby.DataType, ((int)type).ToString());
                        memberOfLobbies.Add(new SteamId(r.m_ulSteamIDLobby));
                    }

                    callback.Invoke(new SteamId(r.m_ulSteamIDLobby), e);
                });
            }
            /// <summary>
            /// Removes a metadata key from the lobby.
            ...
            public static LobbyGameServer GetLobbyGameServer(Lobby lobby)
            {
                SteamMatchmaking.GetLobbyGameServer(lobby, out uint ip, out ushort port, out SteamId serverId);
                return new LobbyGameServer
                {
                    id = serverId,
                    ipAddress = ip,
            ...
            /// <param name="lobby"></param>
            /// <returns></returns>
            public static UserData GetLobbyOwner(Lobby lobby) => ((Steamworks.Data.Lobby)lobby).Owner;
            /// <summary>
            /// Invite another user to the lobby.
            /// </summary>
            /// <param name="lobby">The Steam ID of the lobby to invite the user to.</param>
            ...
            /// <param name="lobby">The Steam ID of the lobby to join.</param>
            /// <param name="callback"></param>
            public static void JoinLobby(Lobby lobby, Action<Lobby, bool> callback)
            {
                if (callback == null)
                    return;

                SteamSettings.behaviour.StartCoroutine(FacepunchJoinLobby(lobby, callback));
            }
            private static System.Collections.IEnumerator FacepunchJoinLobby(Lobby lobby, Action<Lobby, bool> callback)
            {
                var task = SteamMatchmaking.JoinLobbyAsync(lobby);
                yield return new WaitUntil(() => task.IsCompleted);

                if (task.Result.HasValue)
                    callback?.Invoke(task.Result.Value, false);
                else
            ...
            /// <param name="port">Sets the connection port of the game server, in host order.</param>
            /// <param name="gameServerId">Sets the Steam ID of the game server. Use k_steamIDNil if you're not setting this.</param>
            public static void SetLobbyGameServer(Lobby lobby, uint ip, ushort port, SteamId gameServerId) => SteamMatchmaking.SetLobbyGameServer(lobby, ip, port, gameServerId);
            /// <summary>
            /// Sets the game server associated with the lobby.
            /// </summary>
            /// <remarks>
            ...
            /// <param name="port">Sets the connection port of the game server, in host order.</param>
            /// <param name="gameServerId">Sets the Steam ID of the game server. Use k_steamIDNil if you're not setting this.</param>
            public static void SetLobbyGameServer(Lobby lobby, string ip, ushort port, SteamId gameServerId) => SteamMatchmaking.SetLobbyGameServer(lobby, Utilities.IPStringToUint(ip), port, gameServerId);
            /// <summary>
            /// Sets whether or not a lobby is joinable by other players. This always defaults to enabled for a new lobby.
            /// </summary>
            /// <remarks>
            ...
            /// <param name="key"></param>
            /// <returns></returns>
            public static string GetLobbyMemberData(Lobby lobby, SteamId member, string key) => SteamMatchmaking.GetLobbyMemberData(lobby, member, key);
            /// <summary>
            /// Get the LobbyMember object for a given user
            /// </summary>
            /// <param name="id">The ID of the member to fetch</param>
            /// <param name="member">The member found</param>
            /// <returns>True if the user is a member of the lobby, false if they are not</returns>
            public static bool GetMember(Lobby lobby, SteamId id, out LobbyMember member)
            {
                var contained = GetLobbyMemberData(lobby, id, "anyKey");
                if (contained == null)
                {
            ...
            /// <param name="id">The user to check for</param>
            /// <returns>True if they are, false if not</returns>
            public static bool IsAMember(Lobby lobby, SteamId id)
            {
                var contained = GetLobbyMemberData(lobby, id, "anyKey");
                return contained != null;
            }
            ...
            /// <param name="newOwner"></param>
            /// <returns></returns>
            public static bool SetLobbyOwner(Lobby lobby, SteamId newOwner) => SteamMatchmaking.SetLobbyOwner(lobby, newOwner);
            /// <summary>
            /// Updates what type of lobby this is.
            /// </summary>
            /// <param name="lobby"></param>
