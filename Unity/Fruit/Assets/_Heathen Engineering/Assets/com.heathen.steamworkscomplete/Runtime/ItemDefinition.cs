...
namespace HeathenEngineering.SteamworksIntegration
{
    ...
    public class ItemDefinition : UnityEngine.ScriptableObject
    {
        public List<Steamworks.InventoryItem> Details => details;
        public long TotalQuantity => details.Sum(p => (long)p.Quantity);
        /// <summary>
        /// This is only useful after calling API.Inventory.Client.LoadItemDefinitions.
        /// This will return the localized name of the item if definitions have been loaded.
        ...
        public bool HasPrice
        {
            get
            {
                return API.Inventory.Client.GetItemPrice(Id, out int _, out int _);
            }
        }
        ...
        public int CurrentPrice
        {
            get
            {
                if (API.Inventory.Client.GetItemPrice(Id, out int current, out int _))
                {
                    return current;
                }
                else
        ...
        public int BasePrice
        {
            get
            {
                if (API.Inventory.Client.GetItemPrice(Id, out int _, out int baseprice))
                {
                    return baseprice;
                }
                else
        ...
        [NonSerialized]
        private readonly List<Steamworks.InventoryItem> details = new List<Steamworks.InventoryItem>();
        ...
        public class LanguageVariantNode
        {
            ...
            public void Populate(InventoryDef itemDefId)
            {
                value = itemDefId.GetProperty(node);
                ...
                for (int i = 0; i < languages.Length; i++)
                {
                    var r = itemDefId.GetProperty(node + "_" + languages[i]);
                    if (!string.IsNullOrEmpty(r))
                    {
                        variants.Add(new LanguageVariant
                        {
        ...
        public class PromoRule
        {
            ...
            public List<AppId> owns = new List<AppId>();
            public List<string> achievements = new List<string>();
            public List<PlayedEntry> played = new List<PlayedEntry>();
            public bool manual = false;
            ...
            public override string ToString()
            {
                ...
                for (int i = 0; i < owns.Count; i++)
                {
                    ...
                    sb.Append("owns:" + owns[i].Value.ToString());
                }
            ...
            public void Populate(InventoryDef itemDefId)
            {
                ...
                var results = itemDefId.GetProperty("promo");
                ...
                if (!string.IsNullOrEmpty(results))
                {
                    ...
                    {
                        if (rule.StartsWith("owns:"))
                            owns.Add(new AppId { Value = uint.Parse(rule.Replace("owns:", string.Empty)) });
                        else if (rule.StartsWith("ach:"))
                            achievements.Add(rule.Replace("ach:", string.Empty));
                        else if (rule.StartsWith("played:"))
                        {
                            ...
                            if (split.Length > 1)
                                played.Add(new PlayedEntry
                                {
                                    app = new AppId{ Value = uint.Parse(split[0])},
                                    minutes = uint.Parse(split[1])
                                });
                            else
                                played.Add(new PlayedEntry
        ...
        public class Price
        {
            ...
            public void Populate(InventoryDef itemDefId)
            {
                var result = itemDefId.GetProperty("price_category");
                if (!string.IsNullOrEmpty(result))
                {
                    usePricing = true;
                    useCategory = true;
                ...
                {
                    result = itemDefId.GetProperty("price");
                    if (!string.IsNullOrEmpty(result))
                    {
                        usePricing = true;
                        useCategory = false;
        ...
        public class TagCollection
        {
            ...
            {
                tags = new List<ItemTag>();
                var result = itemDefId.GetProperty("tags");
        ...
        public class TagGeneratorValues
        {
            ...
            {
                values = new List<TagGeneratorValue>();
                var result = itemDefId.GetProperty("tag_generator_values");
        ...
        {
            get => new Steamworks.Data.InventoryDefId { Value = item_itemdefid };
#if UNITY_EDITOR
            set
            {
                item_itemdefid = value.Value;
            }
#endif
        }
        [SerializeField]
        ...
        [SerializeField]
        internal Bundle item_bundle = new Bundle();
        public AppId[] PromoRuleOwns => item_promo.owns.ToArray();
        public string[] PromoRuleAchievements => item_promo.achievements.ToArray();
        public PromoRule.PlayedEntry[] PromoRulePlayed => item_promo.played.ToArray();
        [SerializeField]
        internal PromoRule item_promo = new PromoRule();
        ...
        /// <param name="callback"></param>
        /// <returns></returns>
        public void AddPromoItem(Action<InventoryResult?> callback) => API.Inventory.Client.AddPromoItem(Id, callback);
        public ConsumeOrder[] GetConsumeOrders(int quantity)
        {
            if (TotalQuantity < quantity)
                return null;
            ...
            while (count < quantity)
            {
                int cCount = Unity.Mathematics.math.min(details[index].Quantity, quantity - count);
                count += cCount;
        ...
        /// </summary>
        /// <returns></returns>
        public bool Consume(Action<InventoryItem[]> callback)
        {
            if (TotalQuantity <= 0)
                return false;

            var instance = details.First(p => p.Quantity > 0);
            API.Inventory.Client.ConsumeItem(instance, 1, callback);
            return true;
        }
        /// <summary>
        /// Consumes a consume order, you can get consume orders for multiple instances via the <see cref="GetConsumeOrders(uint)"/> method.
        ...
        /// <param name="order"></param>
        /// <param name="callback"></param>
        public void Consume(ConsumeOrder order, Action<InventoryItem[]> callback) => API.Inventory.Client.ConsumeItem(order.detail, order.quantity, callback);
        /// <summary>
        /// Gets a set of exchange entries required to satisfy a given quantity in exchange
        /// </summary>
        /// <param name="quantity"></param>
        /// <param name="entries"></param>
        /// <returns></returns>
        public bool GetExchangeEntry(int quantity, out InventoryItem.Amount[] entries)
        {
            if (TotalQuantity < quantity)
            {
                entries = new InventoryItem.Amount[0];
                return false;
            }
            else
            {
                var list = new List<InventoryItem.Amount>();
                int count = 0;
                var index = 0;
                while (count < quantity)
                {
                    if (details[index].Quantity <= quantity - count)
                    {
                        if (details[index].Quantity > 0)
                        {
                            list.Add(new InventoryItem.Amount
                            {
                                Item = details[index],
                                Quantity = details[index].Quantity
                            }); ;
        ...
        /// <param name="recipeEntries"></param>
        /// <param name="callback"></param>
        public void Exchange(IEnumerable<InventoryItem.Amount> recipeEntries, Action<InventoryItem[]> callback)
        {
            var list = recipeEntries.ToArray();

            API.Inventory.Client.ExchangeItems(list, Id, callback);
        }
        /// <summary>
        /// Generates a single instance of this item
        /// </summary>
        ...
        /// </remarks>
        /// <param name="callback"></param>
        public void GenerateItem(Action<InventoryItem[]> callback)
        {
            API.Inventory.Client.GenerateItems(new Steamworks.Data.InventoryDefId[] { Id }, new int[] { 1 }, callback);
        }
        /// <summary>
        /// Generates a number of this item
        /// </summary>
        ...
        /// <param name="quantity"></param>
        /// <param name="callback"></param>
        public void GenerateItem(int quantity, Action<InventoryItem[]> callback)
        {
            API.Inventory.Client.GenerateItems(new Steamworks.Data.InventoryDefId[] { Id }, new int[] { quantity }, callback);
        }
        /// <summary>
        /// Starts the purchase process for the user, given a shopping cart of item definitons that the user would like to buy
        /// </summary>
        /// <param name="callback"></param>
        public void StartPurchase(Action<Steamworks.Data.InventoryPurchaseResult?> callback)
        {
            API.Inventory.Client.StartPurchase(new Steamworks.Data.InventoryDefId[] { Id }, new uint[] { 1 }, callback);
        }
        /// <summary>
        /// Starts the purchase process for the user, given a shopping cart of item definitons that the user would like to buy
        /// </summary>
        public void StartPurchase(uint count, Action<Steamworks.Data.InventoryPurchaseResult?> callback)
        {
            API.Inventory.Client.StartPurchase(new Steamworks.Data.InventoryDefId[] { Id }, new uint[] { count }, callback);
        }
        /// <summary>
        /// After a successful call to RequestPrices, you can call this method to get the pricing for a specific item definition.
        /// </summary>
        ...
        /// <param name="basePrice"></param>
        /// <returns></returns>
        public bool GetPrice(out int currentPrice, out int basePrice) => API.Inventory.Client.GetItemPrice(Id, out currentPrice, out basePrice);
        /// <summary>
        /// Trigger an item drop if the user has played a long enough period of time.
        /// </summary>
        /// <param name="callback"></param>
        public void TriggerDrop(Action<InventoryItem[]> callback) => API.Inventory.Client.TriggerItemDrop(Id, callback);
        ...
        /// <param name="entries"></param>
        /// <returns></returns>
        public bool CanExchange(ExchangeRecipe recipie, out List<InventoryItem.Amount> entries)
        {
            if (!recipie.Valid)
            {
                entries = null;
            ...
            entries = new List<InventoryItem.Amount>();
            foreach (var mat in recipie.materials)
            {
                if (mat.item.item == null)
                {
                ...
                {
                    if (mat.item.item.GetExchangeEntry(mat.item.count, out InventoryItem.Amount[] found))
                    {
                        entries.AddRange(found);
                    }
                    else
