...
namespace HeathenEngineering.SteamworksIntegration
{
    public class ItemShoppingCartManager : MonoBehaviour
    {
        ...
        {
            if (OrderPending
                && appId == SteamSettings.ApplicationId
                && orderId == OrderId)
            {
        ...
        public string TotalPriceSymbolledString()
        {
            ...
            return SteamSettings.Client.inventory.LocalCurrencySymbol + (total * 0.01d).ToString("0.00");
        }
        ...
        {
            var total = TotalPrice();
            return (total * 0.01d).ToString("0.00") + " " + SteamSettings.Client.inventory.LocalCurrencyCode.ToString();
        }
